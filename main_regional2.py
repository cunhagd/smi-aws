...
def importar_portais(base_diretorio, categoria="regionais"):
    """
    Importa dinamicamente todos os módulos Python dentro de `portais/regionais`.
    """
    portais = []
    # Define o caminho específico para portais/regionais a partir do base_diretorio
    ...
    # Percorre apenas o diretório portais/regionais
...
def main():
    """
    Função principal que executa a raspagem dos 97 portais regionais restantes.
    """
    try:
        base_diretorio = os.path.dirname(os.path.abspath(__file__))
        print(f"Base diretório: {base_diretorio}")
        
        # Importa todos os portais da categoria "regionais"
        todos_portais = importar_portais(base_diretorio, "regionais")
        
        # Pega os 97 portais restantes (após o 100º)
        portais = todos_portais[100:]
        ...
        relatorio = f"""
        Relatório de Execução (Regionais - Parte 2):
        --------------------------------------------
        ...
        print(f"Erro durante a execução da main_regional2: {e}")
