...
def extrair_dados_noticia(url):
    ...
    autor_texto = "Redação Por Dentro de Tudo"

    data = soup.select_one("time")
    data_texto = formatar_data(data.get_text(strip=True)) if data else print("Data não encontrada " + url)
...
def main():
    nome_portal = "Por Dentro de Tudo"
    acesso = AcessarCodigoFonte(url, usar_headers=True)
    html = acesso.acessar()
