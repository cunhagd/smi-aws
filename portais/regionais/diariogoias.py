...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        ...
        if (
            url.count('-') > 2
            and url.count('/') > 3
            and '/cidades/' not in url
            and '/tabela-e-classificacao-do-' not in url
...
def extrair_dados_noticia(url):
    ...
    corpo = soup.select("div.col-12.col-lg-8.pb-4 p")
