...
def formatar_data(data_str):
    try:
        ...
        print("Erro ao definir o locale para pt_BR. Verifique se o locale está instalado no sistema.")
        return "Data não encontrada"
...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("div.check-roles h1")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"

    autor = soup.select_one('div.f1.article__author-info a.article__author-name')
    autor_texto = autor.get_text(strip=True) if autor else 'Redação Jornal GGN'

    data = soup.select_one('div.fc.article__time time')
    data_texto = formatar_data(data.get_text(strip=True)) if data else "Data não encontrada"

    corpo = soup.select("div.f1.article__content p")
