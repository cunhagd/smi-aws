...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        ...
        if (
            url.count('/') > 2
            and url.count('-') > 3
            and url.startswith('https://imparcialnews.com.br/')
        ):
...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one("meta[itemprop='author']")
    autor_texto = autor['content'] if autor else "Redação Imparcial News"

    data = soup.select_one("time[itemprop='datePublished']")
    data_texto = formatar_data(data['datetime']) if data else print(f"Data não encontrada {url}")

    corpo = soup.select("div.vw-post-content.clearfix p")
