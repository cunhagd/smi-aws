...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        url = link["href"]
        if (
            url.startswith('https://jornalfloripa.com.br/')
            and url.count('/') > 2
            and '/noticia/' in url
        ):
...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one(".author.vcard a")
    autor_texto = autor.get_text(strip=True) 
    if autor_texto == "admin":
        autor_texto = "Redação Jornal Floripa" 
    else:
        "Redação Jornal Floripa"

    data = soup.select_one(".entry-date")
    ...
    if corpo_paragrafos:
        ultimo_paragrafo = corpo_paragrafos[-1].strip()
    if ultimo_paragrafo.lower().startswith("fonte:"):
        autor_texto = ultimo_paragrafo.split("Fonte:")[1].strip()
    elif "com informações da" in ultimo_paragrafo.lower():
        autor_texto = ultimo_paragrafo.split("Com informações da")[1].strip()

    if autor_texto.lower() == "admin":
        autor_texto = "Redação Jornal Floripa"
