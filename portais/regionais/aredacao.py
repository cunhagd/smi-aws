...
def extrair_dados_noticia(url):
    ...
    autor_texto = "A Redação"

    data = soup.select_one('span.olho span.right')
    data_texto = formatar_data(data.get_text().split('|')[-1].strip()) if data else "Data não encontrada"

    corpo = soup.select("span.convert-emoji.text p, span.convert-emoji.text div, span.convert-emoji.text span, span.convert-emoji.text strong")
    corpo_paragrafos = [element.get_text(separator=' ', strip=True) for element in corpo]
