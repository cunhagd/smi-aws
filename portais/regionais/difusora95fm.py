...
def formatar_data(data_str):
    try:
        ...
        return "Data não encontrada", "Hora não encontrada"
...
def extrair_dados_noticia(url):
    ...
    autor_texto = "Redação Difusora 95 FM"
    
    data = soup.select_one('p.data')
    data_texto = formatar_data(data.get_text(strip=True).replace(' às ', ' ')) if data else "Data não encontrada"

    corpo = soup.select("div.cont-interna p")
