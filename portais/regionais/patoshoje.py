...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1.details-title.mb-15")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"

    autor = soup.select_one(".col-5 span")
    autor_texto = autor.get_text(strip=True).replace("Por ", "") if autor else "Redação PatosHoje"   

    data = soup.select_one(".col-5 small")
    data_texto = formatar_data(data.get_text(strip=True)) if data else "Data não encontrada"

    corpo = soup.select("div.post-content p")
