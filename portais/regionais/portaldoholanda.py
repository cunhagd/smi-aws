...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one("p.autor")
    autor_texto = autor.get_text(strip=True).replace("Por ", "") if autor else "Redação Portal do Holanda"

    data = soup.select_one("p.destaque.ha")
    data_texto = formatar_data(data.get_text(strip=True).split(" ")[0]) if data else "Data não encontrada"

    corpo = soup.select("div.corpo.corpo1 p")
