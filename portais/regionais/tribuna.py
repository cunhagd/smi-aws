...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        url = link["href"]
        if (
            f"/{dia_atual:02}-{mes_atual:02}-{ano_atual:04}/" in url
            and "/noticias/" in url
        ):
...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one("div.meta p a strong")
    data = soup.select_one("div.meta p.updated")
    corpo = soup.select("div.content.entry-content p")
    ...
    if autor:
        ...
        autor_texto = "Redação Tribuna de Minas"
