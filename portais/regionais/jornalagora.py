...
def extrair_dados_noticia(url):
    ...
    autor_texto = autor.get_text(strip=True) if autor else "Redação Jornal Agora"

    data = soup.select_one('time.entry-date')
    data_texto = formatar_data(data['datetime']) if data else "Data não encontrada"

    corpo = soup.select("div.post-entry.blockquote-style-1 p")
