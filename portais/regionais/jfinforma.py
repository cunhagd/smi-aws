...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        url = link["href"]
        if (
            url.startswith("https://jfinforma.com/")
            and f"/{ano_atual:04}/{mes_atual:02}/{dia_atual:02}/" in url
        ):
...
def formatar_data(data_str):
    try:
        ...
        data_obj = datetime.strptime(data_str, "%Y-%m-%dT%H:%M:%S%z")
...
def extrair_dados_noticia(url):
    ...
    autor_texto = autor["content"] if autor else "Redação JF Informa"
    ...
    corpo = soup.select("div.bs-blog-post.single p")
