...
def extrair_dados_noticia(url):
    ...
    autor_texto = autor.find('a').get_text() if autor and autor.find('a') else "Redação Âncora News"

    data = soup.find('small', {'class': 'data-noticia'})
    data_texto = formatar_data(data.get_text().split(' – ')[0]) if data else "Data não encontrada"
