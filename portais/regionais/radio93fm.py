...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one('div.author-date .author')
    autor_texto = autor.get_text(strip=True) if autor else "Redação Rádio 93 FM"

    data = soup.select_one('div.author-date span:not(.author)')
    data_texto = data.get_text(strip=True) if data else "Data não encontrada"

    paragrafos = soup.select('div.fluidTxt p')
