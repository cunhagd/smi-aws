...
def extrair_links(html, base_url):
    ...
    for link in soup.find_all("a", href=True):
        ...
        if(
            url_completa.count('-') > 2
            and url_completa.startswith("https://jmonline.com.br/")
        ):
...
def formatar_data(data_str):
    try:
        data_str = data_str.replace("Publicado em", "").strip()
        data_obj = datetime.strptime(data_str.split(" às ")[0], "%d/%m/%Y")
...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1.styled__HeadingOne-sc-fdx3oi-0.guVcJH")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"

    autor_element = soup.select_one("div.fVeZAW > span.isnzsd")
    autor_texto = autor_element.get_text(strip=True) if autor_element and autor_element.get_text(strip=True) else "Redação JM Online"

    data = soup.select_one("div.gTAOxp > span.lmfmIt")
    data_texto = formatar_data(data.get_text(strip=True)) if data else "Data não encontrada"

    corpo = soup.select("div.Block__Component-sc-1uj1scg-0.gMDwwG p")
