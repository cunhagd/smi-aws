...
def formatar_data(data_str):
    try:
        data_publicada = data_str.strip().replace("Published ", "").strip()
...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1.s-title, h1.s-title.fw-headline")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"

    autor = soup.select_one("span.meta-el.meta-author a")
    autor_texto = autor.get_text(strip=True) if autor else "Redação Tudo em Dia"

    data = soup.select_one("time.date.published")
    data_texto = formatar_data(data.text.strip()) if data else "Data não encontrada"

    corpo = soup.select("div.entry-content.rbct.clearfix.is-highlight-shares p")
