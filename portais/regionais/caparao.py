...
def formatar_data(data_str):
    try:
        ...
        data_obj = datetime.strptime(data_partes, '%d/%m/%Y')
...
def extrair_dados_noticia(url):
    ...
    autor_texto = "Redação Portal Caparaó"

    data = soup.select_one('div.row.noticia-data-e-comp .col-sm-12.col-md-6')
    data_texto = data.get_text(strip=True) if data else "Data não encontrada"
    data_texto = formatar_data(data_texto)

    corpo = soup.select("article.noticia-texto p")
