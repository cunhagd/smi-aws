...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1.cm-entry-title")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"

    autor = soup.select_one("span.cm-author.cm-vcard a.url.fn.n")
    if autor:
        autor_texto = autor.get_text().strip()
    else:
        autor_texto = "Redação Nova 93FM"

    data = soup.select_one("time.entry-date.published.updated")
    if data:
        data_texto = formatar_data(data.get_text())
    else:
        data_texto = "Data não encontrada"

    corpo = soup.select("div.cm-entry-summary p")
