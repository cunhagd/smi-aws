...
def extrair_dados_noticia(url):
    ...
    autor = soup.find('meta', {'name': 'smartbanner:author'})
    autor_texto = autor['content'].strip() if autor and autor['content'].strip() else "Redação Política Livre"

    data = soup.select_one('span.data-postagem')
    data_texto = formatar_data(data.get_text().split('|')[0].strip()) if data else "Data não encontrada"

    corpo = soup.select("div.post-singular p")
...
def main():
    nome_portal = "BLog Política Livre"
    acesso = AcessarCodigoFonte(url, usar_headers=True)
    html = acesso.acessar()
