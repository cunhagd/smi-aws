...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one('div[dir="ltr"] span')
    autor_texto = autor.get_text(strip=True) if autor else "Redação Gazeta Norte"

    data = soup.select_one('.date-cad')
    data_texto = formatar_data(data.get_text(strip=True)[3:]) if data else "Data não encontrada"

    corpo = soup.select("div.texto-news p")
