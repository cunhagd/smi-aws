...
def formatar_data(data_str):
    ...
    try:
        data_formatada = datetime.strptime(data_str, "%d de %m de %Y").strftime("%d/%m/%Y")
...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("div.nazrul-single-blog-title h2")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"

    data = soup.select_one("div.nazrul-blog-meta-left span")
    data_texto = formatar_data(data.get_text(strip=True)) if data else "Data não encontrada"
    
    autor_texto = "Redação Pioneiros"

    corpo = soup.select("div.single-blog-content p")
