...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        ...
        if (
            ...
            or f"/{ano_atual:04}/{mes_atual:02}/" in url
...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1.content-head__title")
    autor = soup.select_one('div.content-publication-data__from span[itemprop="name"]')
    data = soup.select_one("time")
    corpo = soup.select("p.content-text__container")
    ...
    if autor:
        ...
        if "—" in autor_texto:
            autor_texto = autor_texto.split("—")[0].strip()
    ...
    data_texto = formatar_data(data.get('datetime')) if data else "Data não encontrada"
