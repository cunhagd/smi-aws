...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1.jeg_post_title")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"

    autor = soup.select_one("div.jeg_meta_author a")
    autor_texto = autor.get_text(strip=True) if autor else "Redação Terra Brasil Notícias"

    data = soup.select_one("div.jeg_meta_date a")
    data_texto = formatar_data(data.text.strip()) if data else "Data não encontrada"

    corpo = soup.select("div.content-inner p")
    corpo_paragrafos = []
    for p in corpo:
        for sub in p.find_all("sub"):
            sub.decompose()
        corpo_paragrafos.append(p.get_text(separator=' ', strip=True))
...
def main():
    nome_portal = "Terra Brasil Notícias"
    acesso = AcessarCodigoFonte(url)
    html = acesso.acessar()
