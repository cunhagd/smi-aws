...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one("div.td-post-author-name a")
    autor_texto = autor.get_text(strip=True) if autor else "Redação Jornal S'Passo"
        
    data = soup.select_one(".td-post-date time.entry-date.updated.td-module-date")
    data_texto = formatar_data(data.get_text(strip=True)) if data else "Data não encontrada"

    corpo = soup.select("div.td-post-content.tagdiv-type p")
