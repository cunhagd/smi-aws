...
url = "https://www.correiobraziliense.com.br/busca/minas%20gerais/page/1/"
portal_chave = 'braziliense_mg'
...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        ...
        if (
            url.startswith('https://www.correiobraziliense.com.br/')
            and f"/{ano_atual:04}/{mes_atual:02}/" in url
...
def formatar_data(data_str):
    ...
    try:
        data_str = data_str.split(' ')[2]
        dia, mes, ano = data_str.split('/')
        return f'{(dia):02}/{(mes):02}/{(ano):04}'
...
def extrair_dados_noticia(url):
    ...
    if autor:
        ...
        autor_texto = "Redação Correio Braziliense"

    data = soup.select_one("div.date")
    ...
    for p in corpo:
        for a in p.find_all('a'):
            a.decompose()
        corpo_paragrafos.append(p.get_text(separator=' ', strip=False))
