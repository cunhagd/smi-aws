...
def formatar_data(data_str):
    try:
        data_str = data_str.replace("Sexta,", "").strip()
        ...
        dia = partes[0]
        mes_extenso = partes[1]
        ano = partes[2]
        mes = meses.get(mes_extenso.lower())
        if mes is None:
            raise ValueError(f"Mês inválido: {mes_extenso}")
...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1 span#uc_materiaVis_lbl_nmMateriaAssunto")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"

    autor_texto = "Redação GV News"

    data = soup.select_one("p.data-midia-lista")
    data_texto = formatar_data(data.get_text(strip=True)) if data else "Data não encontrada"

    corpo = soup.select("div.conteudo p")
