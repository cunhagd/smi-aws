...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        url = link["href"]
        if (
            f"/{dia_atual}{mes_atual}{ano_atual}/" in url
        ):
...
def extrair_dados_noticia(url):
    ...
    autor_element = soup.select_one('span.ag-article-meta meta[itemprop="name"]')
    autor_texto = autor_element.get("content", "Autor não encontrado") if autor_element else 'Redação GRNews'

    data = soup.select_one('div.single-post-meta time.single-post-date')
