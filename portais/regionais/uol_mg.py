...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one("a.solar-author-name")
    corpo = soup.select_one("div.jupiter-paragraph-fragment")
    link = soup.select_one("link[rel='canonical']")

    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"
    autor_texto = autor.get_text(strip=True) if autor else "Redação UOL"
