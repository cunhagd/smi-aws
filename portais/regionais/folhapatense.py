...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        ...
        if (
            url.count('-') > 3
            and url.count('/') > 2
            and url.startswith("https://www.folhapatense.com.br/") 
        ):
...
def extrair_dados_noticia(url):
    ...
    autor_texto = autor.get('content') if autor else "Redação Folha Patense"
    ...
    corpo = soup.select("div.post-text.mt-4 p")
