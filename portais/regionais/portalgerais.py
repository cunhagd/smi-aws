...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one("span.meta-author a")
    autor_texto = autor.get_text(strip=True) if autor else "Redação Portal Gerais"

    data = soup.select_one("span.meta-date.date.published")
    data_texto = formatar_data(data.get_text(strip=True)) if data else "Data não encontrada"

    corpo = soup.select("div.post-inner p")
