...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        ...
        if (
            url.count('/') > 2
            and url.count('-') > 3
            and url.startswith('https://www.saogoncaloagora.com.br/')
        ):
...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1.titulo-post")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"

    autor = soup.select_one(".info-autor span")
    if autor:
        autor_texto = autor.get_text(strip=True)
        autor_texto = autor_texto.split(":")[-1].strip()

        if autor_texto.lower() == "são gonçalo agora":
            fonte = soup.select_one(".info-autor span:nth-of-type(2)")
            if fonte:
                autor_texto = fonte.get_text(strip=True).split(":")[-1].strip()
    else:
        autor_texto = "Redação São Gonçalo Agora"

    data = soup.select_one(".data-post span")
