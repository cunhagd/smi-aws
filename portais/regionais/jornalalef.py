...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        ...
        if (
            ...
            and "/assistir-ao-vivo-" not in url
            and "/resultado-do-sorteio-" not in url
        ):
...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one(".author-meta a")
    autor_texto = autor.get_text(strip=True) 
    if autor_texto == "admin" or autor_texto == "":
        autor_texto = "Redação Jornal Agora"

    data = soup.select_one("span.posted-date")
    data_texto = formatar_data(data.get_text(strip=True)) if data else "Data não encontrada"

    corpo = soup.select("div.single-entry-summary p")
