...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        ...
        if (
            url.count('/') > 2
            and url.count('-') > 3
            and url.startswith("https://www.jornalbairrosnet.com.br/")
        ):
...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one(".cm-author.cm-vcard a")
    autor_texto = autor.get_text(strip=True) if autor else "Redação Bairros Net"
