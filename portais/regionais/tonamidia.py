...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h4.elementor-heading-title.elementor-size-default")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"

    autor = soup.select_one("span.elementor-icon-list-text.elementor-post-info__item.elementor-post-info__item--type-author")
    if autor:
        autor_texto = autor.get_text(strip=True).split(" | ")[0]
    else:
        autor_texto = "Redação To na Mídia"

    data = soup.select_one("time")
    if data and data.text:
        data_texto = formatar_data(data.text.strip())
    else:
