...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one('li.post-author.notranslate strong')
    autor_texto = autor.get_text(strip=True) if autor else "Redação Rádio 104 FM"

    data = soup.select_one('li .fa-calendar')
    data_texto = formatar_data(data.find_parent().get_text(strip=True)) if data else "Data não encontrada"

    paragrafos = soup.select('div.materia.capitular p')
    corpo_paragrafos = [p.get_text(separator=' ', strip=True) for p in paragrafos]
