...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("div.pageNews h1")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"
    
    autor_texto = "Redação Diário de Uberlândia"
    ...
    corpo = soup.select("p[style*='text-align: justify'] span[style*='font-size:16px']")
...
def main():
    nome_portal = "Diário de Uberlândia"
    acesso = AcessarCodigoFonte(url)
    html = acesso.acessar()
