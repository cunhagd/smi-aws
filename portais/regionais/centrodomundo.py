...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        ...
        if (
            url.count('-') > 2
            and url.count('/') > 2
            and '/web-stories/' not in url
            and '/wp-content/' not in url
            and '/essencial/' not in url
        ):
...
def extrair_dados_noticia(url):
    ...
    titulo_texto = titulo.get_text(strip=True
                                   ) if titulo else "Título não encontrado"

    autor = soup.select_one('meta[name="author"]')  
    autor_texto = autor.get('content', '').strip() if autor else "Redação Diário do Centro do Mundo"
    ...
    corpo = soup.select("div.td-post-content.td-pb-padding-side p")
