...
portal_chave = 'independentenoticias'
...
def formatar_data(data_str):
    try:
        data_obj = datetime.fromisoformat(data_str[:-1]) 
...
def extrair_dados_noticia(url):
    ...
    autor_texto = autor['content'].strip() if autor else "Redação Independente Notícias"
    ...
    corpo = soup.select("article.blog-post-page-font p")
...
def main():
    nome_portal = "Independente Notícias"
    acesso = AcessarCodigoFonte(url)
    html = acesso.acessar()
