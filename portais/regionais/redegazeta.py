...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        ...
        if (
            url.count('/') > 2
            and url.count('-') > 3
            and '/veiculos-e-negocios/' not in url
        ):
...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one("div.col-xs-12.col-sm-6.col-md-7 span")
    autor_texto = autor.get_text(strip=True) if autor else "Redação Rede Gazeta"

    data = soup.select_one("div.col-xs-12.col-sm-6.col-md-7 span:nth-of-type(2)")
