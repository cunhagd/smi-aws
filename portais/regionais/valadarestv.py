...
def formatar_data(data_str):
    try:
        data_sem_hora = data_str.split(" às ")[0]
        dia, mes, ano = data_sem_hora.split("/")
...
def extrair_dados_noticia(url):
    ...
    data_span = soup.select_one("div.data-post span")
    if data_span and data_span.text:
        data_texto = formatar_data(data_span.text.strip())
    else:
        data_texto = "Data não encontrada"

    autor_span = soup.select_one("div.info-autor span")
    if autor_span:
        autor_texto = autor_span.get_text(strip=True).replace("Por: ", "").replace(autor_span.b.get_text(strip=True), "").strip()
    else:
        autor_texto = "Redação Valadares na TV"
...
def main():
    nome_portal = "Plantão de Minas (Valadares na TV)"
    acesso = AcessarCodigoFonte(url)
    html = acesso.acessar()
