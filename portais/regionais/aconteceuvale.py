...
def extrair_dados_noticia(url):
    ...
    autor = soup.find('meta', {'itemprop': 'name'})
    autor_texto = autor['content'] if autor else "Redação A Notícia Online"

    data = soup.find('meta', {'itemprop': 'datePublished'})
    data_texto = formatar_data(data['content']) if data else "Data não encontrada"

    corpo = soup.select("div.td-post-content p")
