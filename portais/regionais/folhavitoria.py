...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        url = link["href"]
        full_url = urljoin("https://www.folhavitoria.com.br/", url)
        
        if (
            f"/{mes_atual:02}/{ano_atual}/" in full_url
            and "/noticia/" in full_url
        ):
...
def extrair_dados_noticia(url):
    ...
    data_texto = data.get('content') if data else "Data não encontrada"
    data_texto = formatar_data(data_texto)

    autor = soup.select_one('meta[property="article:author"]')
    autor_texto = autor.get('content') if autor else "Redação Folha Vitória"

    corpo = soup.select('div[data-offset-key] span[data-text="true"]')
    if not corpo:
        corpo = soup.find_all('p')

    corpo_paragrafos = [p.get_text(strip=True) for p in corpo] if corpo else ["Parágrafo não encontrado"]
