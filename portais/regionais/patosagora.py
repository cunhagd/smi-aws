...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("div.large-8.small-12.columns.principal h1")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"

    info_texto = soup.select_one('.large-9.medium-8.small-12.columns.text-right.size-12.mt10.mb10')
    texto_completo = info_texto.get_text(strip=True)
    autor_texto = info_texto.find('strong').get_text(strip=True) 
    partes = texto_completo.split(" - ")

    data_str = partes[0].split(" ")[1]  
