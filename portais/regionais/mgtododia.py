...
def extrair_dados_noticia(url):
    ...
    if autor:
        autor_texto = autor.get_text(strip=True).split(":")[-1].strip()
        if fonte:
            fonte_texto = fonte.get_text(strip=True).split(":")[-1].strip()
            
            if autor_texto.lower() == fonte_texto.lower():
                autor_texto = autor.get_text(strip=True).split(":")[-1].strip()
            else:
                autor_texto = fonte_texto
        else:
            autor_texto = autor.get_text(strip=True).split(":")[-1].strip()
    else:
        autor_texto = "Redação MG Todo Dia"
