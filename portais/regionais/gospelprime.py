...
def formatar_data(data_str):
    try:
        data_obj = datetime.strptime(data_str.strip(), "%d de %B de %Y")
...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1.mvp-post-title.left.entry-title")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"

    autor = soup.select_one("span.author-name")
    autor_texto = autor.get_text(strip=True).replace("Por ", "") if autor else "Autor não encontrado"
    ...
    corpo = soup.select("div#mvp-content-main p")
