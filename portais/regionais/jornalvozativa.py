...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        ...
        if (
            url.count('/') > 2
            and url.count('-') > 3
            and 'http://www.yummly.com/' not in url
...
def formatar_data(data_str):
    try:
        data_publicada = data_str.split(",")[0].split("Publicado em")[-1].strip()
        data_obj = datetime.strptime(data_publicada, "%d/%m/%Y")
...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1.center.margem-0")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"

    autor = soup.select_one("span.autor span.nome strong")
    if autor:
        autor_texto = autor.get_text(strip=True)
        if ".com" in autor_texto:
            autor_texto = autor_texto.replace(".com", "").strip()
    else:
        autor_texto = "Redação Jornal Voz Ativa"

    data = soup.select_one("div.meta-superior span.nome:-soup-contains('Publicado em')")
    data_texto = formatar_data(data.get_text(strip=True)) if data else "Data não encontrada"

    corpo = soup.select("div.fusion-text p")
