...
def extrair_dados_noticia(url):
    ...
    autor_texto = autor.get_text(strip=True).replace("Por ", "") if autor else "Redação Hotelier News"

    data = soup.select_one("span.date")
    data_texto = formatar_data(data.get_text(strip=True)) if data else "Data não encontrada"

    corpo = soup.select("span.post-content p")
