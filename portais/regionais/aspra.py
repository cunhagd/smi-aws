...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        ...
        if (
            ...
            and "https://api.whatsapp.com/" not in full_url
            and full_url.startswith("https://aspra.org.br/portal/")
        ):
...
def extrair_dados_noticia(url):
    ...
    autor = soup.find('p', {'class': 'autor-noticia'})
    autor_texto = autor.find('a').get_text() if autor and autor.find('a') else "Redação Aspra"

    data = soup.find('time', {'itemprop': 'dateCreated'})
    data_texto = formatar_data(data['datetime']) if data else "Data não encontrada"

    corpo = soup.select('div[itemprop="articleBody"] p')
