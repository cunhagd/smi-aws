...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("div.info_post h3")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"
    
    autor = soup.select_one('div.info_post div > span:-soup-contains("Por")')
    if autor:
        autor_texto = autor.get_text(strip=True).replace("Por", "").strip()
    else:
        autor_texto = "Redação RCWTV"

    data = soup.select_one("time")
    data_texto = formatar_data(data.get_text(strip=True).replace("Em", "").strip()) if data else "Data não encontrada"

    corpo = soup.select("div.post_content p")
