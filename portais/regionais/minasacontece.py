...
def extrair_dados_noticia(url):
    ...
    autor_texto = "Redação Minas Acontece"

    data = soup.select_one("div.elementor-element[data-id='489a88b'] h2.elementor-heading-title")
    data_texto = formatar_data(data.get_text(strip=True)) if data else "Data não encontrada"

    corpo = soup.select("div.jet-listing-dynamic-field__content p")
