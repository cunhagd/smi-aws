...
portal_chave = 'estadao'
url = 'https://www.estadao.com.br/tudo-sobre/minas-gerais-estado/'
dominio_principal = 'https://www.estadao.com.br'
...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("div.container-news-informs h1")
    autor = soup.select_one("div.names")
    data = soup.select_one("div.date time")
    corpo = soup.select("div.styles__ContentWrapperContainerStyled-sc-1ehbu6v-0 p")
    ...
    if autor:
        ...
        autor_texto = autor_texto.replace("Por", "").strip()
        autor_texto = autor_texto.split(",")[0].strip()
    else:
        autor_texto = "Autor não encontrado"
