...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        ...
        if (
            url.count('-') > 2
            and url.count('/') > 3
            and "https://esbrasil.com.br/categoria/diversao-e-arte/lazer/aonde-ir/" not in url
            and url.startswith("https://esbrasil.com.br/")
            and "/diversao-e-arte/" not in url
            and "/colunas/" not in url
        ):
...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one('span.td-page-meta meta[itemprop="name"]')
    autor_texto = autor.get('content', '').strip() if autor else "Redação ES Brasil"

    data = soup.select_one("div.tdb-block-inner.td-fix-index time.entry-date.updated.td-module-date")
