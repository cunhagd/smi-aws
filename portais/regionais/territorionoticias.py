...
portal_chave = 'territorionoticias'
...
def extrair_dados_noticia(url):
    ...
    autor_texto = "Redação Território Notícias"

    data = soup.select_one('span.glyphicon.glyphicon-time')
    data_texto = formatar_data(data.find_parent().get_text(strip=True).replace('às ', ' ').strip()) if data else "Data não encontrada"
        
    corpo = soup.select("div.col-md-9 p")
...
def main():
    nome_portal = "Território Notícias"
    acesso = AcessarCodigoFonte(url)
    html = acesso.acessar()
