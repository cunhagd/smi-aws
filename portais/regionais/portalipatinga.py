...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1 grey-text.text-darken-4.no-margin")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"
    
    autor_texto = "Redação Potal da Cidade Ipatinga"

    data_meta = soup.select_one('meta[itemprop="datePublished"]')
    data_texto = data_meta.get('content') if data_meta else "Data não encontrada"
    data_texto = formatar_data(data_texto)

    corpo = soup.select("div.news-story p")
