...
def extrair_dados_noticia(url):
    ...
    autor_data = soup.select_one("div.autor")
    autor = autor_data.select_one("img")
    autor_texto = autor.get("alt") if autor else "Redação Portal 6"

    data = autor_data.select_one("time")
    if data:
        data_texto = formatar_data(data.get_text(strip=True)) 
    else:
        data_texto = "Data não encontrada"
        
    corpo = soup.select("div.p__single--content p")
