...
def extrair_dados_noticia(url):
    ...
    autor_texto = autor.get('content') if autor else "Redação Betan City"

    data = soup.select_one('meta[property="article:published_time"]')
    data_texto = data.get('content') if data else "Data não encontrada"
    data_texto = formatar_data(data_texto.split('T')[0]) if data else "Data não encontrada"
