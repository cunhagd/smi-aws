...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one('div.td-author-name.vcard.author .fn a')
    autor_texto = autor.get_text(strip=True) if autor else "TV Poços"

    data = soup.find('meta', {'itemprop': 'datePublished'})
    data_texto = formatar_data(data['content']) if data else "Data não encontrada"

    paragrafos = soup.select('div.td-post-content p')
