...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        url = link["href"]
        if (
            url.startswith("https://conexaoitajuba.com.br/")
        ):
...
def extrair_dados_noticia(url):
    ...
    autor_meta = soup.select_one('meta[name="author"]')
    autor_texto = autor_meta.get('content') if autor_meta else "Redação Conexão Itajubá"
    ...
    corpo = soup.select("div.the_content_wrapper p")
