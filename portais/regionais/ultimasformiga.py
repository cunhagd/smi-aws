...
def formatar_data(data_str):
    try:
        ...
        data_part, hora_part = data_str.split(" - ")
        data_obj = datetime.strptime(f"{data_part} {hora_part}", "%d/%m/%y %H:%M")
...
def extrair_dados_noticia(url):
    ...
    autor_texto = 'Redação Últimas Formiga'

    data = soup.select_one('span.updated')
    data_texto = formatar_data(data.get_text(strip=True)) if data else "Data não encontrada"

    corpo = soup.select("div.post-wrap p")
    corpo_paragrafos = [p.get_text(separator=' ', strip=True) for p in corpo[1:]]
    corpo_texto = processador_texto.formatar_corpo(
        corpo_paragrafos) if corpo_paragrafos else "Corpo da notícia não encontrado"
...
def main():
    nome_portal = "Últimas Notícias - Formiga"
    acesso = AcessarCodigoFonte(url)
    html = acesso.acessar()
