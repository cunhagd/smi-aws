...
url = "https://veja.abril.com.br/noticias-sobre/minas-gerais"
portal_chave = 'veja_mg'
...
def formatar_data(data_str):
    ...
    try:
        data_str = data_str.split(",")[0].replace('Atualizado em ', '').replace('Publicado em ', '').strip()
        partes_data = data_str.split()
        if len(partes_data) == 3:
            dia = partes_data[0].zfill(2)
            mes = meses.get(partes_data[1].lower())
            ano = partes_data[2]
            if mes:
...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one("span.author > strong > span") or soup.select_one("span.author > strong > a > span")
    data = soup.select_one("span.date-post")
    corpo = soup.select("section.content p")
