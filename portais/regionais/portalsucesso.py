...
url = "https://web.portalsucesso.com.br/"
portal_chave = 'portalsucesso'
...
def formatar_data(data_str):
    try:
        ...
        dia, mes, ano = data_str.split(" de ")
        mes = meses[mes.lower()]  # Converte o nome do mês para número
...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1.post-title.left")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"

    autor = soup.select_one("span.author-name.vcard.fn a")
    autor_texto = autor.get_text(strip=True) if autor else "Redação Portal Sucesso"

    data = soup.select_one("time.post-date.updated")
    data_texto = formatar_data(data.text.strip()) if data else "Data não encontrada"    

    corpo = soup.select("div.left.relative p")
