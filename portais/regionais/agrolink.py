...
def extrair_dados_noticia(url):
    ...
    autor_texto = autor['content'] if autor else "Redação AgroLink"

    data = soup.find('time', {'itemprop': 'datePublished dateModified'})
    data_texto = formatar_data(data['datetime']) if data else "Data não encontrada"

    corpo = soup.select("div.section-description.section-description-noticias.mt-3 p")
