...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        ...
        if (
            url.count('/') > 2
            and url.count('-') > 3
            and url.startswith('https://horadopovo.com.br/')
        ):
...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one("p a[rel='author']")
    autor_texto = autor.get_text(strip=True).replace("Por ", "") if autor else "Redação Hora do Povo"

    data = soup.select_one("p:-soup-contains('Publicado em')")
    data_texto = formatar_data(data.get_text(strip=True).split("Publicado em ")[-1]) if data else "Data não encontrada"

    corpo = soup.select("div.page-content.sinng p")
