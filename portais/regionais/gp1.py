...
def extrair_links(html, base_url):
    ...
    for link in soup.find_all("a", href=True):
        ...
        if (
            ...
            and "https://www.facebook.com/" not in url
            and "https://wa.me/" not in url
            and "https://t.me/" not in url
            and "https://api.addthis.com/" not in url
            and "https://www.linkedin.com/" not in url
            and "https://twitter.com/" not in url
...
def formatar_data(data_str):
    try:
        ...
        data_bruta = data_str.split('|')[0].strip()
...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one('div.jornalista.perfil a strong')
    autor_texto = autor.get_text(strip=True) if autor else 'Redação GP1'

    data = soup.select_one('div.jornalista.jornalista-time time')
    data_texto = formatar_data(data.get_text(strip=True)) if data else 'Data não encontrada'

    corpo = soup.select("div.article-texto p")
