...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1.single-title.entry-title")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"

    autor = soup.select_one("span.meta-info-el.meta-info-author a")
    autor_texto = autor.get_text(strip=True) if autor else "Redação Web Terra"

    data = soup.select_one("abbr.date.published")
    data_texto = formatar_data(data['title']) if data else "Data não encontrada"
