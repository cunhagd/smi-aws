...
def formatar_data(data_str):
    try:
        ...
        data_part = ' '.join(data_str.split()[1:]) 
        data_part = data_part.replace('º', '')
        if 'de' in data_part and len(data_part.split()) == 3:
            data_formatada = datetime.strptime(data_part, '%d de %B').strftime('%d/%m')
            data_formatada = f"{data_formatada}/{datetime.now().year}"
        else:
            data_formatada = datetime.strptime(data_part, '%d de %B de %Y').strftime('%d/%m/%Y')
