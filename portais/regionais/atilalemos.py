...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1.g1-mega.g1-mega-1st.entry-title")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"

    autor = soup.find('meta', {'name': 'twitter:data1'})
    autor_texto = autor['content'] if autor else "Redação Átila Lemos"
