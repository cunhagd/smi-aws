...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1.post-title.entry-title")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"

    autor = soup.select_one("div.author-info h3.author-name a")
    autor_texto = autor.get_text(strip=True) if autor else "Redação Mais Top News"

    data = soup.select_one("span.meta-item.last-updated")
    data_texto = formatar_data(data.get_text(strip=True).replace("Última Atualização ", "")) if data else "Data não encontrada"

    corpo = soup.select("div.entry-content.entry.clearfix p")
