...
def formatar_data(data_str):
    try:
        data_obj = datetime.strptime(data_str, "%d de %B de %Y, %H:%M")
...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one('span.entry-author a')
    autor_texto = autor.get_text(strip=True) if autor else "Redação Itabira Net"

    data = soup.select_one("time.entry-date")
    data_texto = formatar_data(data.get_text(strip=True)) if data else "Data não encontrada"

    corpo = soup.select("div.g1-content-narrow.g1-typography-xl.entry-content p")
