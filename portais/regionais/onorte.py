...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one("span.styled__Span-sc-fdx3oi-7.efTqTR")
    if not autor:
        autor = soup.select_one("span.styled__Span-sc-fdx3oi-7.bLaGAa")
    autor_texto = autor.get_text(strip=True) if autor and autor.get_text(strip=True) else "Autor não encontrado"

    data = soup.select_one("div.Block__Component-sc-1uj1scg-0.goIqbO span.styled__Span-sc-fdx3oi-7.gQSNuO")
    data_texto = formatar_data(data.get_text(strip=True)) if data else "Data não encontrada"

    corpo = soup.select("div.Block__Component-sc-1uj1scg-0.hMHEKK p")
