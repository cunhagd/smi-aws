...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1.s-title.fw-headline")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"

    autor = soup.select_one(".meta-el.meta-author a")
    autor_texto = autor.get_text(strip=True) if autor else "Redação Regionalzão"
    
    data = soup.select_one(".updated-date")
    data_texto = formatar_data(data['datetime']) if data else "Data não encontrada"
    if data_texto == "Data não encontrada":
        ano_atual = datetime.now().year
        mes_atual = datetime.now().month
        dia_atual = datetime.now().day
        data_texto = f'{dia_atual}/{mes_atual}/{ano_atual}'

    corpo = soup.select("div.entry-content.rbct.clearfix p")
