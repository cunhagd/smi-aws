...
def formatar_data(data_str):
    try:
        ...
        data = datetime.strptime(data_str, "%B %d, %Y %H:%M")
...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1.m-t-xs-20.m-b-xs-0.axil-post-title.hover-line")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"

    autor = soup.select_one("div.post-metas.banner-post-metas.m-t-xs-20 span")
    if autor:
        autor_texto = autor.get_text(strip=True)
    else:
        autor_texto = "Redação Novo Jornal"

    data = soup.select_one("span.published.updated")
    if data:
        data_texto = formatar_data(data.get_text())
    else:
        data_texto = "Data não encontrada"

    corpo = soup.select("div.row p")
