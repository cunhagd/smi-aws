...
def formatar_data(data_str):
    try:
        ...
        data_formatada = datetime.strptime(data_part, '%d/%m/%Y').strftime('%d/%m/%Y')
...
def extrair_dados_noticia(url):
    ...
    autor_element = soup.select_one('div.pos-info-post .info-post:nth-of-type(3) span b')
    autor_texto = autor_element.get_text(strip=True) if autor_element else "Redação Folha Sete Lagoas"

    data_element = soup.select_one('div.pos-info-post .data span')
    data_texto = formatar_data(data_element.get_text(strip=True)) if data_element else "Data não encontrada"
...
def main():
    nome_portal = "Folha de Sete Lagoas"
    acesso = AcessarCodigoFonte(url)
    html = acesso.acessar()
