...
def extrair_dados_noticia(url):
    ...
    autor_texto = "Redação JC Notícias"

    data = soup.select_one('div.cont_pagina p.color-policial')
    data_texto = formatar_data(data.get_text(strip=True)) if data else "Data não encontrada"

    corpo = soup.select('div.texto_redactor p')
