...
def formatar_data(data_str):
    try:
        data_str = data_str.strip().split(' ')[0] 
...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one("div.post-article__header__author span") 
    autor_texto = autor.get_text(strip=True) if autor else "Redação Primeira Página"

    data = soup.select_one("p.post-article__info__date time")
    data_texto = formatar_data(data.get_text(strip=True)) if data else "Data não encontrada"

    corpo = soup.select("div.post-article__content__main.grid__col--4--sm.grid__col--8--md.grid__col--12--lg p")
