...
def formatar_data(data_str):
    data_str = data_str.replace("em ", "").split(" às ")[0]
...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1.h1.tit-not")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"
    
    autor = soup.select_one("span.author")
    if autor:
        autor_texto = autor.get_text(strip=True).split(" ")[1]
    else:
        autor_texto = "Redação São Paulo Jornal"
    
    data = soup.select_one("span.date")
    data_texto = formatar_data(data.text.strip()) if data else "Data não encontrada"

    corpo = soup.select("div.materia.capitular p")
