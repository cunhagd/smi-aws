...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        url = link["href"]
        if (
            (f"/{ano_atual:04}/{mes_atual:02}/" or f"/{ano_atual:04}/{mes_atual}/") in url
...
def formatar_data(data_str):
    ...
    try:
        mes_abreviado, dia, ano = data_str.replace(",", "").split()
        mes = meses.get(mes_abreviado.title())
