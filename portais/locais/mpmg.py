...
def extrair_links(html):
    ...
    links_noticias = set()
    dominio_principal = "https://www.mpmg.mp.br" 
...
def formatar_data(data_str):
    try:
        data_limpa = data_str.replace("Publicado em", "").strip().split()[0]
        data_formatada = datetime.strptime(data_limpa, "%d/%m/%y").strftime("%d/%m/%Y")
...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("div.page-header h1")
    if titulo:
        span = titulo.find("span", class_="category")
        if span:
            span.extract()
        titulo_texto = titulo.get_text(strip=True)
    else:
        titulo_texto = "Título não encontrado"

    autor_texto = "Assessoria de Comunicação Integrada"

    data = soup.select_one("div.meta-data ul.meta-date-time li")
    if data:
        data_publicacao = data.get_text(strip=True).replace("Publicado em", "").split()[0]
        data_texto = formatar_data(data_publicacao)
    else:
        data_texto = "Data não encontrada"

    corpo = soup.select("main#mpmg_conteudo p")
