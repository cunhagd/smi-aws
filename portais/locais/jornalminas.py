...
def formatar_data(data_str):
    ...
    try:
        dia, mes_abreviado = data_str.split('/')
        mes = meses.get(mes_abreviado.lower())  
        if mes:
            ano_atual = datetime.now().year
            data_formatada = f"{dia.zfill(2)}/{mes}/{ano_atual}"
...
def extrair_dados_noticia(url):
    ...
    autor_texto = "Redação Jornal Minas"

    data = soup.select_one("div.data")
