...
def formatar_data(data_str):
    try:
        if "Publicado em" in data_str:
            data_str = data_str.replace("Publicado em", "").strip()
        data_partes = data_str.split(' ')[0]
        data_formatada = datetime.strptime(data_partes, "%d/%m/%Y").strftime("%d/%m/%Y")
...
def extrair_dados_noticia(url):
    ...
    corpo_paragrafos = [p.get_text(strip=True) for p in corpo[2:]]
