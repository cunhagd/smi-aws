...
def extrair_links(html):
    ...
    for link in soup.find_all('a', href=True):
        ...
        if (
            ...
            and url.count('-') > 2
            and url.count('/') > 2
            and '/campinas-e-regiao/' not in url
            and '/rio-de-janeiro/' not in url
            and '/olimpiadas/' not in url
            and '/esportes/' not in url
            and '/minas-gerais/' in url
            and '/colunistas/' not in url
        ):
...
def extrair_dados_noticia(url):
    ...
    data = soup.select_one("span.author__date-hour time")
    if data and data.has_attr('datetime'):
        data_texto = formatar_data(data['datetime'])
    else:
        data_texto = "Data não encontrada"
    
    autor_texto ="Redação Band Minas"

    corpo = soup.select("app-article-text div.text p") 
