...
def extrair_links(html):
    ...
    links_noticias = set()
    dominio_principal = "https://www.gazetadopovo.com.br/"
...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1.post-title")
    autor = soup.select_one("span.author-name")
    
    # Selecionando os elementos <li> da lista <ul class="wgt-date">
    data_elements = soup.select("ul.wgt-date li")
    
    # Tenta buscar a data no segundo <li>
    data_texto = "Data não encontrada"
    if len(data_elements) > 1:
        data_texto = formatar_data(data_elements[1].get_text(strip=True))
    
    # Se a data não for encontrada no segundo <li>, tenta buscar no primeiro <li>
    if data_texto == "Data não encontrada" and len(data_elements) > 0:
        data_texto = formatar_data(data_elements[0].get_text(strip=True))

    corpo = soup.select("p[tabindex='0']")
    ...
    if autor:
        ...
        autor_texto = "Redação Gazeta"

    # Extraindo os textos dos parágrafos encontrados com o seletor
