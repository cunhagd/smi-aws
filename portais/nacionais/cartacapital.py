...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one("div.s-content__infos a[title]") 
    autor_texto = autor.get_text(strip=True) if autor else "Redação Carta Capital"

    data_meta = soup.select_one('meta[property="article:published_time"]')
    data_texto = formatar_data(data_meta.get('content')) if data_meta else "Data não encontrada"

    corpo = soup.select("div.contentOpen p")
