...
portal_chave = 'infomoney'
    
def extrair_links(html):
    soup = BeautifulSoup(html, "html.parser")
    links_noticias = set()
    dominio_principal = "https://www.infomoney.com.br/"
...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1[class*='text-3xl']") 
    autor = soup.select_one("a[class*='text-base']") 
    
    data_element = soup.select_one("time[datetime]")
    data_texto = data_element["datetime"] if data_element else "Data não encontrada"
    data_texto = formatar_data(data_texto)

    corpo = soup.select(".im-article p")
