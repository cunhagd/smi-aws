...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        url = link["href"]
        if (
            url.startswith("https://istoe.com.br/")
            and url.count("/") > 3
            and url.count("-") > 3
        ):
...
def formatar_data(data_texto):
    ...
    try:
        data_partes = data_texto.split('-')[0].strip()
        dia, mes, ano = data_partes.split('/')
...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("section.section-materia h1")
    autor = soup.select_one("p.author.tooltip-comp")
    data = soup.select_one("div.texts > p:nth-of-type(2)")
    corpo = soup.select("section.conteudo p")
