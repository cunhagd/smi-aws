...
def formatar_data(data_str):
    try:
        partes_data = data_str.strip().split(" ")[0] 
        data_formatada = datetime.strptime(partes_data, "%d/%m/%Y").strftime("%d/%m/%Y")
...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1.content-title")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"

    autor = soup.select_one("div.single-author-body a p")
    autor_texto = autor.get_text(strip=True) if autor else "Redação Canal Rural"

    data = soup.select_one("time span")
    data_texto = "Data não encontrada"
    if data:
        data_str = data.get_text(strip=True)
        data_texto = formatar_data(data_str)

    corpo_paragrafos = soup.select("div.single-content.mb-4 p")
