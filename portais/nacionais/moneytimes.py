...
portal_chave = 'moneytimes'
    
def extrair_links(html):
    soup = BeautifulSoup(html, "html.parser")
    links_noticias = set()
    dominio_principal = "https://www.moneytimes.com.br"
...
def extrair_dados_noticia(url, formatar_data):
    ...
    titulo = soup.select_one("h1.single_title")  
    autor = soup.select_one("a.single_meta_author_infos_name_title")  
    
    data_element = soup.select_one("span.single_meta_author_infos_date_time")
    data_texto = data_element.get_text(strip=True) if data_element else "Data não encontrada"
    data_texto = formatar_data(data_texto)

    corpo = soup.select("div.single_block_news_text p")
    ...
    if autor:
        ...
        autor_texto = "Redação Money Times"
...
def main():
    ...
    for link in links_noticias:
        try:
            dados_noticia = extrair_dados_noticia(link, formatar_data)
