...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1.content-title.text-gray-900.m-0") 
    if not titulo:
        titulo = soup.select_one("p.content-title.text-white.text-uppercase.m-0") 

    autor = soup.select_one("p.content-author.text-primary.m-0 a")
    data = soup.select_one("time.content-infopost")
    corpo = soup.select("div.single-content.px-4.px-lg-0 p")
    ...
    if autor:
        ...
        autor_texto = "Redação The Intercept"
