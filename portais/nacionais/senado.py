...
def extrair_links(html):
    ...
    dominio_base = "https://www12.senado.leg.br"  
    ...
    for link in soup.find_all("a", href=True):
        ...
        if(
            "/noticias/materias/" in url
            and f"{ano_atual}/{mes_atual}/{dia_atual:02}" in url
        ):
            full_url = urljoin(dominio_base, url)
...
def formatar_data(data_str):
    try:
        ...
        data_formatada = datetime.strptime(data_parte, "%d/%m/%Y")
...
def extrair_dados_noticia(url):
    ...
    autor = soup.select_one("small")
    data = soup.select_one("span.text-muted")
    corpo = soup.select("div#textoMateria p")
    ...
    if autor:
        ...
        if autor_texto.lower().startswith("da "):
            autor_texto = autor_texto[3:].strip()
    else:
        autor_texto = "Redação Senado"
