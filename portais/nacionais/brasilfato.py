...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        url = link["href"]
        full_url = urljoin("https://brasildefato.com.br", url)
        
        if (
            f"/{ano_atual:04}/{mes_atual:02}/{dia_atual:02}/" in full_url
            and "/esportes/" not in full_url
        ):
...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("h1.title[itemprop='headline']")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"
    
    autor = soup.select_one("p.editor[rel='editor'][itemprop='editor']")
    autor_texto = "Redação Brasil de Fato"
    if autor:
        autor_texto = autor.get_text(strip=True).replace("Edição:", "").strip()

    data = soup.select_one("time.date[itemprop='datePublished']")
    data_texto = "Data não encontrada"
    if data and 'datetime' in data.attrs:
        data_texto = formatar_data(data['datetime'])

    corpo = soup.select("div.text-content p")
