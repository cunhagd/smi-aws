...
def extrair_links(html):
    ...
    for link in soup.find_all("a", href=True):
        ...
        if (
            url.count('/') > 3
            and "/topics/" not in url
            and url.startswith("https://www.bbc.com/portuguese")
...
def extrair_dados_noticia(url):
    ...
    titulo = soup.select_one("div.bbc-1151pbn.ebmt73l0 h1.bbc-14gqcmb.e1p3vdyi0")
    titulo_texto = titulo.get_text(strip=True) if titulo else "Título não encontrado"
    
    data = soup.select_one("time.bbc-1dafq0j")
    data_texto = "Data não encontrada"
    if data and 'datetime' in data.attrs:
        data_texto = formatar_data(data['datetime'])

    autor = soup.select_one("span.bbc-1ypcc2")
    autor_texto = autor.get_text(strip=True) if autor else "Redação BBC"

    corpo = soup.select("p.bbc-hhl7in")
